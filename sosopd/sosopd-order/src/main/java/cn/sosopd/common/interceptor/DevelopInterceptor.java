...
public class DevelopInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (StringUtils.isNotEmpty(uri)) {
			String uriLast = StringUtils.substringAfterLast(uri, "/");
			if (StringUtils.contains(uriLast, ';')) {
				uriLast = StringUtils.substringBefore(uriLast, ";");
			}
			if (StringUtils.isNotEmpty(uriLast) && !StringUtils.contains(uriLast, ".")) {
				LOGGER.warn("可能使用了错误的请求URL，没有添加正确的后缀（如.html或者.json）：" + uri);
			}
		}
		return true;
	}
