...
public class TokenController {
	...
	@RequestMapping(value = SysConstants.REQUEST_URL_PREFIX + "/login.json", method = RequestMethod.POST)
	@IgnoreSecurity
	@ResponseBody
	public Response login(@RequestParam("account") String uname, @RequestParam("password") String passwd,
		...
		if (userAuthService.checkAccount(uname, passwd) != null) {
			...
			Cookie cookie = new Cookie(SysConstants.DEFAULT_TOKEN_NAME, token);
			cookie.setPath("/");
	...
	@RequestMapping(value = SysConstants.REQUEST_URL_PREFIX + "/login2.json", method = RequestMethod.POST)
	@IgnoreSecurity
	@ResponseBody
	...
	@RequestMapping(value = SysConstants.REQUEST_URL_PREFIX + "/quit.json", method = RequestMethod.POST)
	@ResponseBody
	public Response quit(HttpServletRequest request) {

		request.getSession().removeAttribute(SysConstants.DEFAULT_USER_SESSION_NAME);
