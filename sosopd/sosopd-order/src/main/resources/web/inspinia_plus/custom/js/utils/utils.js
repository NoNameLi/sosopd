 ...
 * @license utils.js 1.0.01 Copyright (c) 2017, Tangqy All Rights Reserved.
 * Available via the MIT or new BSD license.
 * see: https://github.com/Tonyee for details
 * 
 * 工具类，集合了各种系统中使用的操作方法
 * 
 * @date 2016-05-31
...
function formatDate(date,pattern){
	...
	// 根据pattern格式化日期串
	if(pattern){
		return pattern.replace(/yyyy/,y).replace(/yy/,y1).replace(/MM/,M2).replace(/M/,M)
...
/**
 * 某个时间在当前时间的多久前
 * @param e：即为某个时间的时间戳或日期对象 
 * @param t：是否在超过30天后，只返回日期字符，而不返回时分秒 
 * @description  如果在3分钟以内，返回：刚刚 
...
function timeAgo(e, t) {
	...
	return i > 6912e5 ? (i = new Date(e), o[0][0] = i.getFullYear(), o[0][1] = putOnTens(i.getMonth() + 1), o[0][2] = putOnTens(i.getDate()), t || (o[1][0] = putOnTens(i.getHours()), o[1][1] = putOnTens(i.getMinutes()), o[1][2] = putOnTens(i.getSeconds())), o[0].join("-") + " " + o[1].join(":")) : i >= 864e5 ? (i / 1e3 / 60 / 60 / 24 | 0) + "天前": i >= 36e5 ? (i / 1e3 / 60 / 60 | 0) + "小时前": i >= 12e4 ? (i / 1e3 / 60 | 0) + "分钟前": i < 0 ? "未来": "刚刚"
}
// 个位数首位加“0”
function putOnTens(val){
...
function checkDateStr(dateStr){
	...
	//return isNaN(dateStr)?(isNaN(time)?false:true):false;
